...
const uiJS = `
 ...
 if(typeof document!=='undefined'){
  ...
  // act drives the mutating routes behind the stop/continue buttons. On success
  // it drops the fragment cache so the very next poll re-renders even when the
  // server bytes have not changed yet; on refusal it shows the server's
  // plain-text reason and re-enables the button.
  window.act=function(btn){var a=btn.getAttribute('data-act'),n=btn.getAttribute('data-issue');btn.disabled=true;
   fetch('/'+a+'?issue='+n,{method:'POST'}).then(function(r){
    if(r.status===204){lastDetail=null;lastRail=null;window.poll();return;}
