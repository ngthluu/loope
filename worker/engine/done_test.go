...
func TestParseAlreadyDone(t *testing.T) {
	cases := []struct {
		...
		// Reproduces the issue #73 false-positive: a fix's own explanation quotes
		// the sentinel constant name mid-sentence (e.g. describing the gate it
		// just fixed) without emitting it as a real terminal signal on its own
		// line. A naive substring search treats that mention as the sentinel and
		// wrongly closes the issue as already-implemented even though a real fix
		// was committed.
		{"quoted mid-sentence, not a real signal", "Fixed and committed. Root cause: afterDebug only " +
			"gated the UAT step on `CONFIDENCE:`/`PIPELINE_ALREADY_DONE:` sentinels in the debug session's output.",
			false, ""},
	}
