...
// Deps are the ports the Orchestrator runs against. main (the composition
// root) fills them with the infra adapters; tests fill them with fakes. The
// engine only ever sees these interfaces — it never imports infra.
type Deps struct {
	Cfg       *shared.Config
...
// NewOrchestrator wires an Orchestrator from its ports.
func NewOrchestrator(d Deps) *Orchestrator {
	return &Orchestrator{cfg: d.Cfg, gh: d.Host, wt: d.Workspace, newAgent: d.NewAgent, images: d.DownloadImages}
}
...
type Orchestrator struct {
	...
	wt       shared.Workspace
	newAgent func(logDir string) shared.Agent
	images   func(ctx context.Context, content, destDir string) string
...
// (never started, already finished, or a double Stop) — a no-op, surfaced by the
...
func (o *Orchestrator) handleIssue(ctx context.Context, issue shared.Issue, kind, base string) error {
	...
	priorState := shared.ReadState(logDir)
	...
	if o.images != nil {
		content = o.images(ctx, content, logDir)
	}
	...
	var perr error
	if node, ok := shared.ResumePoint(logDir); ok {
		...
		if node.Stage == shared.StageCodeReview {
...
func (o *Orchestrator) finishDone(ctx context.Context, n int, reason string) error {
	...
	shared.RecordState(o.issueLogDir(n), o.cfg.StateLabels.Done)
	shared.ClearParkCause(o.issueLogDir(n))
...
func (o *Orchestrator) finishNeedsInfo(ctx context.Context, n int, lc *lowConfidenceError) error {
	...
	shared.RecordState(o.issueLogDir(n), o.cfg.StateLabels.NeedsInfo)
	shared.ClearParkCause(o.issueLogDir(n))
	return nil
}
...
func classifyCause(msg string) (guidance string) {
	...
	for _, sig := range shared.TransientSignatures {
...
func (o *Orchestrator) park(ctx context.Context, n int, cause error) error {
	cctx := context.WithoutCancel(ctx)
	_ = o.gh.Comment(cctx, n, parkComment(o.cfg.StateLabels.Rework, classifyCause(cause.Error()), shared.Clip(cause.Error(), 6000)))
	_ = o.gh.SwapLabels(cctx, n, o.cfg.StateLabels.WIP, o.cfg.StateLabels.Rework)
	shared.RecordState(o.issueLogDir(n), o.cfg.StateLabels.Rework)
	shared.RecordParkCause(o.issueLogDir(n), cause.Error())
	return cause
}
...
func (o *Orchestrator) pause(ctx context.Context, n int) {
	...
	shared.RecordState(logDir, o.cfg.StateLabels.Stopped)
...
func (o *Orchestrator) ship(ctx context.Context, issue shared.Issue, c shared.Agent, wtPath, branch, base, kind string) error {
	...
	alreadyAnnounced := shared.HasPR(logDir)
	...
	shared.RecordState(logDir, o.cfg.StateLabels.Done)
	shared.ClearParkCause(logDir)
	return nil
}
