...
// (clearing inProgress). Handler calls run under testkit.FakeRunner's lock, so the
...
func newMergeEnv(t *testing.T, labels ...string) *mergeEnv {
	t.Helper()
	env := &mergeEnv{f: &testkit.FakeRunner{}, workDir: t.TempDir(), labels: labels, claudeResolves: true}
	if err := os.MkdirAll(shared.WorktreePath(env.workDir, 7), 0o755); err != nil {
		t.Fatal(err)
	}
...
func (e *mergeEnv) orchestrator() *Orchestrator {
	cfg := &shared.Config{
		...
		WorkDir:           e.workDir, StateLabels: testStateLabels(),
		Models: shared.Models{Architect: shared.ModelConfig{Model: "opus", Effort: "high"}},
	}
	o := newTestOrch(cfg, e.f)
	return o
}
...
func TestMergeResolveConflictRunsOneClaudeSessionThenPushes(t *testing.T) {
	...
	if want := shared.WorktreePath(env.workDir, 7); claudes[0].Dir != want {
		t.Errorf("resolve session dir = %q, want the worktree %q", claudes[0].Dir, want)
	}
	if !testkit.HasArg(claudes[0].Args, "--dangerously-skip-permissions") {
...
func TestMergeResolveDoesNotTouchPipelineSession(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(env.logDir(), shared.SessionFile), []byte(seed), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := runMergeCycle(env.orchestrator()); err != nil {
		t.Fatal(err)
	}
	got, err := os.ReadFile(filepath.Join(env.logDir(), shared.SessionFile))
	if err != nil {
		t.Fatal(err)
	}
	if string(got) != seed {
...
func TestMergeResolveUnresolvedConflictParksWithoutPush(t *testing.T) {
	...
	if cause := shared.ReadParkCause(env.logDir()); !strings.Contains(cause, "did not conclude") {
...
func TestMergeResolveMissingWorktreeParks(t *testing.T) {
	env := newMergeEnv(t, "ai-agent", "ai-done", "ai-resolve-merge")
	if err := os.RemoveAll(shared.WorktreePath(env.workDir, 7)); err != nil {
		t.Fatal(err)
	}
	if err := runMergeCycle(env.orchestrator()); err != nil {
		t.Fatal(err)
	...
	if cause := shared.ReadParkCause(env.logDir()); !strings.Contains(cause, "no worktree") {
