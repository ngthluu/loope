...
type flowEnv struct {
	...
	claude func(c rcall) (string, string, error)
}
...
func newFlowEnv(t *testing.T) *flowEnv {
	...
	workDir := t.TempDir()
	env := &flowEnv{
		t: t, f: &fakeRunner{}, workDir: workDir,
		wt:     worktreePath(workDir, flowIssue),
...
func (e *flowEnv) orchestrator() *Orchestrator {
	cfg := &Config{
		...
		WorkDir: e.workDir, MaxQARounds: 3, StateLabels: defaultStateLabels(),
		Models: Models{
			Architect:  ModelConfig{Model: "opus", Effort: "high"},
			Answerer:   ModelConfig{Model: "sonnet"},
			Triage:     ModelConfig{Model: "sonnet"},
			CodeReview: &CodeReviewConfig{ModelConfig: ModelConfig{Model: "opus"}, Rounds: 1},
		},
	}
...
func (e *flowEnv) logDir() string {
	return filepath.Join(e.workDir, "logs", shared.IssueDirName(flowIssue))
}
...
func (e *flowEnv) claudeResumes(id string) []string {
	...
	var out []string
	for _, c := range e.f.calls {
		if c.name == "claude" && argAfter(c.args, "--resume") == id {
			out = append(out, c.stdin)
		}
	}
	return out
...
func (e *flowEnv) realChain() []SessionNode {
	var out []SessionNode
	for _, n := range readSessionChain(e.logDir()) {
		if n.ID != "" {
			out = append(out, n)
		}
	}
...
func (e *flowEnv) featureScript() func(c rcall) (string, string, error) {
	return func(c rcall) (string, string, error) {
		switch {
		case strings.Contains(c.stdin, "triage agent"):
			return claudeJSON(fmt.Sprintf(`{"issueNumber": %d, "kind": "feature", "reason": "small"}`, flowIssue), "t1"), "", nil
		...
		case strings.Contains(c.stdin, "/code-review"):
			return claudeJSON(codeReviewBeginSentinel+"\nSTATUS: clean\nNothing to fix.\n"+codeReviewEndSentinel, "cr-1"), "", nil
		}
		return claudeJSON("ok", "eph-1"), "", nil
	}
}
...
func TestFlowFeatureHappyPath(t *testing.T) {
	...
	if got := readState(env.logDir()); got != "ai-done" {
	...
	wantStages := []string{stageBrainstorm, stagePlan, stageExecute, stageCodeReview}
	...
	if head, _ := headSession(env.logDir()); head.Stage != stageCodeReview {
...
func TestFlowExecuteKilledParksThenReworkRemovalResumesSameSession(t *testing.T) {
	...
	killed := false
	env.claude = func(c rcall) (string, string, error) {
		...
		if argAfter(c.args, "--resume") == "exec-dead" {
			return claudeJSON("Executed after resume.", "exec-2"), "", nil
		}
		return happy(c)
	}
	o := env.orchestrator()
	...
	if head, _ := headSession(env.logDir()); head.ID != "exec-dead" || head.Stage != stageExecute {
...
func TestFlowPlanDiesBeforeSessionStartsResumesFreshFromSpec(t *testing.T) {
	...
	if node, ok := resumePoint(env.logDir()); !ok || node.ID != "" || node.Stage != stagePlan {
...
func TestFlowBugUsageLimitParksThenResumes(t *testing.T) {
	...
	env.claude = func(c rcall) (string, string, error) {
		switch {
		case strings.Contains(c.stdin, "triage agent"):
			return claudeJSON(fmt.Sprintf(`{"issueNumber": %d, "kind": "bug", "reason": "crash"}`, flowIssue), "t1"), "", nil
		case strings.Contains(c.stdin, "/code-review"):
			return claudeJSON(codeReviewBeginSentinel+"\nSTATUS: clean\nok\n"+codeReviewEndSentinel, "cr-1"), "", nil
		case argAfter(c.args, "--resume") == "s-429":
			return claudeJSON("Fixed and committed.", "debug-2"), "", nil
		case !limited:
			limited = true
			return claudeErrorJSON("Claude AI usage limit reached", "s-429"), "", nil
		}
		return claudeJSON("ok", "eph-1"), "", nil
	}
	o := env.orchestrator()
	...
	if head, _ := headSession(env.logDir()); head.ID != "s-429" || head.Kind != "bug" || head.Stage != stageDebug {
