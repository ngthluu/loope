...
// errFake is a canned failure for gh doubles.
...
func TestScanLogs(t *testing.T) {
	...
	tickets, err := scanLogs(&shared.Config{WorkDir: work, StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}})
	if err != nil {
		t.Fatal(err)
	}
	if len(tickets) != 1 {
...
func TestScanLogsNoDir(t *testing.T) {
	tickets, err := scanLogs(&shared.Config{WorkDir: t.TempDir(), StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}) // no logs/ subdir
	if err != nil {
...
func TestBuildTicketsMergesLabels(t *testing.T) {
	...
	r := &testkit.FakeRunner{Queue: []testkit.RResp{{Stdout: trackedListJSON()}}}
...
func TestLocalStateWinsOverGitHub(t *testing.T) {
	...
	shared.RecordState(dir, "ai-done")
	...
	shared.ClearState(dir)
	tickets, _ = BuildTickets(context.Background(), &testkit.FakeRunner{Queue: []testkit.RResp{{Stdout: trackedListJSON()}}}, cfg)
	for i := range tickets {
...
// shape, the one integration seam testkit.FakeRunner otherwise leaves unchecked.
func TestBuildTicketsGHArgv(t *testing.T) {
	...
	args := r.Calls[0].Args
	if !testkit.HasArg(args, "issue") || !testkit.HasArg(args, "list") {
		t.Fatalf("args missing issue list: %v", args)
	}
	if got := testkit.ArgAfter(args, "--repo"); got != "o/r" {
	...
	if got := testkit.ArgAfter(args, "--search"); got != "label:ai-agent" {
		t.Fatalf("--search = %q, want label:ai-agent (eligible-scoped)", got)
	}
	if got := testkit.ArgAfter(args, "--state"); got != "all" {
		t.Fatalf("--state = %q, want all", got)
	}
	if !testkit.HasArg(args, "--limit") {
		t.Fatalf("args missing --limit: %v", args)
	}
	if got := testkit.ArgAfter(args, "--json"); got != "number,title,labels" {
...
func TestBuildTicketsGHArgvNoEligibleLabel(t *testing.T) {
	work := t.TempDir()
	cfg := &shared.Config{WorkDir: work, RepoSlug: "o/r", StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}
	r := &testkit.FakeRunner{Queue: []testkit.RResp{{Stdout: "[]"}}}

	if _, ghErr := BuildTickets(context.Background(), r, cfg); ghErr != nil {
		t.Fatalf("unexpected gh error: %v", ghErr)
	}
	if got := testkit.ArgAfter(r.Calls[0].Args, "--search"); got != "label:ai-wip,ai-done,ai-rework,ai-stopped" {
...
func TestBuildTicketsGHFallback(t *testing.T) {
	...
	cfg := &shared.Config{WorkDir: work, RepoSlug: "o/r", EligibleLabel: "ai-agent", StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}
	r := &testkit.FakeRunner{Queue: []testkit.RResp{{Stderr: "gh: offline", Err: errFake}}}
...
func TestRecordPRWritesFile(t *testing.T) {
	dir := t.TempDir()
	shared.RecordPR(dir, "https://github.com/o/r/pull/3")
...
func TestParkCauseRoundTrip(t *testing.T) {
	...
	if got := shared.ReadParkCause(dir); got != "" {
		t.Errorf("missing file should read empty, got %q", got)
	}
	shared.RecordParkCause(dir, "claude execute: terminated: max_turns")
	if got := shared.ReadParkCause(dir); got != "claude execute: terminated: max_turns" {
		t.Errorf("readParkCause = %q", got)
	}
	shared.ClearParkCause(dir)
	if got := shared.ReadParkCause(dir); got != "" {
		t.Errorf("cleared cause should read empty, got %q", got)
	}
	// Best-effort like the other writers: no panic on empty inputs.
	shared.RecordParkCause("", "x")
	shared.RecordParkCause(dir, "")
	shared.ClearParkCause("")
}
...
func TestRecordTitleRoundTrip(t *testing.T) {
	...
	shared.RecordTitle(dir, "Enhance: add Stop/Continue")
	...
	shared.RecordTitle(dir, "")
	shared.RecordTitle("", "x")
...
func TestOverlayIssuesKeepsPersistedTitle(t *testing.T) {
	cfg := &shared.Config{EligibleLabel: "ai-agent", StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}
	tickets := []Ticket{{Number: 3, Title: "Enhance: add Stop/Continue", StateLabel: "ai-done"}}
	got := overlayIssues(tickets, []shared.Issue{{Number: 3, Labels: []shared.Label{{Name: "ai-agent"}}}}, cfg)
...
func TestSortTicketsDoneSinksToBottom(t *testing.T) {
	cfg := &shared.Config{EligibleLabel: "ai-agent", StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}
	sl := shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}
	tickets := []Ticket{
		mkTicket(21, sl.Done),
		mkTicket(2, sl.WIP),
		mkTicket(13, sl.Done),
...
func TestSortTicketsCustomLabelsHonored(t *testing.T) {
	cfg := &shared.Config{
		...
		StateLabels: shared.StateLabels{
			WIP:    "in-progress",
			Done:   "shipped",
			Rework: "revise",
		},
...
func TestReadState(t *testing.T) {
	dir := t.TempDir()
	if got := shared.ReadState(dir); got != "" {
		t.Errorf("readState on an empty dir = %q, want empty", got)
	}
	shared.RecordState(dir, "ai-needs-info")
	if got := shared.ReadState(dir); got != "ai-needs-info" {
