...
func newTestServer(t *testing.T) *Server {
	...
	cfg := &shared.Config{WorkDir: work, RepoSlug: "o/r", EligibleLabel: "ai-agent", StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}
	r := &testkit.FakeRunner{Queue: []testkit.RResp{{Stdout: `[{"number":142,"title":"Add OAuth login","labels":[{"name":"ai-wip"}]}]`}}}
	s, err := NewServer(r, cfg, infra.NewGitHubWithRetry(r, cfg, testkit.TestRetry), nil)
	if err != nil {
		t.Fatal(err)
	}
	return s
...
func TestDetailRouteRendersFragment(t *testing.T) {
	cfg := &shared.Config{RepoPath: "/tmp", RepoSlug: "o/r", WorkDir: t.TempDir(), StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}, EligibleLabel: "ai-agent"}
	fr := &testkit.FakeRunner{}
	s, err := NewServer(fr, cfg, infra.NewGitHubWithRetry(fr, cfg, testkit.TestRetry), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestServeEmptyQueueRenders(t *testing.T) {
	cfg := &shared.Config{WorkDir: t.TempDir(), RepoSlug: "o/r", EligibleLabel: "ai-agent", StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}
	r := &testkit.FakeRunner{Queue: []testkit.RResp{{Stdout: "[]"}}}
	s, err := NewServer(r, cfg, infra.NewGitHubWithRetry(r, cfg, testkit.TestRetry), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestServeGitHubUnreachableRenders(t *testing.T) {
	...
	writeStep(t, dir, 1, "architect", "design the thing", "the design output",
		`{"result":"the design output","session_id":"a3f9","is_error":false,"total_cost_usd":0.51}`)
	cfg := &shared.Config{WorkDir: work, RepoSlug: "o/r", EligibleLabel: "ai-agent", StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}
	r := &testkit.FakeRunner{Queue: []testkit.RResp{{Err: errors.New("could not connect")}}}
	s, err := NewServer(r, cfg, infra.NewGitHubWithRetry(r, cfg, testkit.TestRetry), nil)
	if err != nil {
		t.Fatal(err)
	}
...
// testkit.FakeRunner too, and only queues a single response, so tests can assert
// on how many gh calls the cache allowed through.
func newTestServerWithRunner(t *testing.T) (*Server, *testkit.FakeRunner) {
	t.Helper()
	work := t.TempDir()
	dir := filepath.Join(work, "logs", "issue-142")
	if err := os.MkdirAll(dir, 0o755); err != nil {
...
func TestBackfillPRCachesForSelectedTicket(t *testing.T) {
	work := t.TempDir()
	cfg := &shared.Config{RepoPath: "/tmp", RepoSlug: "o/r", WorkDir: work, StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}, EligibleLabel: "ai-agent"}
	f := &testkit.FakeRunner{Queue: []testkit.RResp{{Stdout: `{"url":"https://github.com/o/r/pull/5"}`}}}
	s, err := NewServer(f, cfg, infra.NewGitHubWithRetry(f, cfg, testkit.TestRetry), nil)
	if err != nil {
		t.Fatal(err)
	}
	tk := &Ticket{Number: 5}
...
func TestDurationFormat(t *testing.T) {
	...
	for in, want := range cases {
		if got := shared.Duration(in); got != want {
			t.Errorf("shared.Duration(%d) = %q, want %q", in, got, want)
		}
	}
}
...
func railTitleEnv(t *testing.T, num int, title string) (*Server, *testkit.FakeRunner) {
	t.Helper()
	work := t.TempDir()
	...
	writeStep(t, dir, 1, "architect", "design the thing", "the design output",
		`{"result":"the design output","session_id":"a3f9","is_error":false,"total_cost_usd":0.51}`)
	mustWrite(t, filepath.Join(dir, shared.StateFile), "ai-done")
	cfg := &shared.Config{WorkDir: work, RepoPath: "/clone", RepoSlug: "o/r", EligibleLabel: "ai-agent", StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}
	r := &testkit.FakeRunner{Handler: func(c testkit.RCall) (string, string, error) {
...
func TestServeTitleBackfillPersistsAndIsNotRepeated(t *testing.T) {
	...
	body, err := os.ReadFile(filepath.Join(s.cfg.WorkDir, "logs", "issue-3", shared.TitleFile))
	if err != nil {
...
func TestServeUsesPersistedTitleWhenGitHubUnreachable(t *testing.T) {
	...
	writeStep(t, dir, 1, "architect", "design the thing", "the design output",
		`{"result":"the design output","session_id":"a3f9","is_error":false,"total_cost_usd":0.51}`)
	mustWrite(t, filepath.Join(dir, shared.TitleFile), "Add OAuth login")
	cfg := &shared.Config{WorkDir: work, RepoSlug: "o/r", EligibleLabel: "ai-agent", StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}
	r := &testkit.FakeRunner{Handler: func(testkit.RCall) (string, string, error) { return "", "", errors.New("could not connect") }}
	s, err := NewServer(r, cfg, infra.NewGitHubWithRetry(r, cfg, testkit.TestRetry), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestServePersistsFetchedTitles(t *testing.T) {
	...
	body, err := os.ReadFile(filepath.Join(s.cfg.WorkDir, "logs", "issue-142", shared.TitleFile))
	if err != nil {
...
//
//	tailwindcss -i web/tailwind.css -o web/static/app.css --minify (run from worker/)
func TestAppCSSCoversBothClassSources(t *testing.T) {
	css, err := webFS.ReadFile("static/app.css")
	if err != nil {
		t.Fatal(err)
	}
	if len(css) < 4096 {
	...
	for _, want := range []string{
		...
		if !strings.Contains(string(css), want) {
			t.Fatalf("app.css missing %q — regenerate it: tailwindcss -i web/tailwind.css -o web/static/app.css --minify (run from worker/)", want)
		}
	}
}
...
func serverWithOrch(t *testing.T, state string, session bool) (*Server, *fakeEnv) {
	...
	shared.RecordState(logDir, state)
	o := newOrch(cfg, env.f)
	s, err := NewServer(env.f, cfg, infra.NewGitHubWithRetry(env.f, cfg, testkit.TestRetry), o)
	if err != nil {
		t.Fatal(err)
	}
	return s, env
...
func TestStopRouteInvokesOrchestratorAndRenders(t *testing.T) {
	...
	// Nothing is in flight, so Stop returns engine.ErrNotRunning and we render an inline
	// notice — not a 5xx.
...
func TestStateKindMapsStopped(t *testing.T) {
	cfg := &shared.Config{StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}, EligibleLabel: "ai-agent"}
...
func TestPickStateLabelPrefersStoppedOverEligible(t *testing.T) {
	cfg := &shared.Config{StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}, EligibleLabel: "ai-agent"}
	labels := []shared.Label{{Name: "ai-agent"}, {Name: "ai-stopped"}}
...
func TestDetailRendersContinueButtonForStopped(t *testing.T) {
	...
	shared.RecordState(dir, "ai-stopped")
	cfg := &shared.Config{WorkDir: work, RepoSlug: "o/r", EligibleLabel: "ai-agent", StateLabels: shared.StateLabels{WIP: "ai-wip", Done: "ai-done", Rework: "ai-rework", NeedsInfo: "ai-needs-info", Stopped: "ai-stopped"}}
	r := &testkit.FakeRunner{Queue: []testkit.RResp{{Stdout: `[{"number":142,"title":"Add OAuth","labels":[{"name":"ai-agent"},{"name":"ai-stopped"}]}]`}}}
	s, err := NewServer(r, cfg, infra.NewGitHubWithRetry(r, cfg, testkit.TestRetry), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadSetsWorktreeFieldsWhenDirExists(t *testing.T) {
	s := newTestServer(t)
	wt := shared.WorktreePath(s.cfg.WorkDir, 142)
