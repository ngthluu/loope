...
type Server struct {
	...
	orch   *engine.Orchestrator // mutation endpoints (/stop, /continue); nil for read-only servers

	ttl time.Duration
	now func() time.Time

	mu         sync.Mutex
	ghIssues   []shared.Issue // last good gh result
...
// returns a Server that renders from the given shared.Runner and Config. It errors if
// a template fails to parse.
func NewServer(r shared.Runner, cfg *shared.Config, host shared.CodeHost, orch *engine.Orchestrator) (*Server, error) {
	tmpl, err := template.New("dashboard").Funcs(templateFuncs(cfg)).ParseFS(webFS, "templates/*.html")
	if err != nil {
		return nil, err
	}
	return &Server{runner: r, cfg: cfg, gh: host, orch: orch, tmpl: tmpl, ttl: defaultGHTTL, now: time.Now,
...
func (s *Server) issues(ctx context.Context) ([]shared.Issue, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Server) setWorktree(v *view) {
	...
	path := shared.WorktreePath(s.cfg.WorkDir, v.Selected.Number)
	v.WorktreePath = path
...
func (s *Server) issueLogDir(n int) string {
	return shared.IssueLogDir(s.cfg.WorkDir, n)
}
...
func (s *Server) persistTitles(tickets []Ticket, onDisk map[int]string) {
	for i := range tickets {
		...
		shared.RecordTitle(s.issueLogDir(tk.Number), tk.Title)
	}
}
...
func (s *Server) backfillTitle(ctx context.Context, tk *Ticket) {
	...
	if err != nil {
		if shared.IsTransientGitHubError(err) {
			return
		}
		s.mu.Lock()
	...
	tk.Title = title
	shared.RecordTitle(s.issueLogDir(tk.Number), title)
}
...
func (s *Server) backfillPR(ctx context.Context, tk *Ticket) {
	...
	url, err := s.gh.PRURLForBranch(ctx, shared.BranchName(tk.Number))
	if err != nil {
		if shared.IsTransientGitHubError(err) {
	...
	s.mu.Unlock()
	tk.PRURL = url
	shared.RecordPR(s.issueLogDir(tk.Number), url)
}
