...
func scanLogs(cfg *shared.Config) ([]Ticket, error) {
...
func statusRank(cfg *shared.Config, label string) int {
	if label == "" {
...
func sortTickets(cfg *shared.Config, tickets []Ticket) {
...
func trackedStateLabels(cfg *shared.Config) []string {
...
func pickStateLabel(labels []shared.Label, cfg *shared.Config) string {
...
// listTrackedIssues shells `gh issue list` (through shared.Runner) for this instance's
...
func listTrackedIssues(ctx context.Context, r shared.Runner, repoPath, slug, eligible string, stateLabels []string) ([]shared.Issue, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("gh issue list: %w (stderr: %s)", err, shared.Tail(stderr, 300))
	}
...
func overlayIssues(tickets []Ticket, issues []shared.Issue, cfg *shared.Config) []Ticket {
	...
	for _, is := range issues {
		...
		if !shared.HasLabel(is.Labels, cfg.EligibleLabel) {
			continue
		}
...
func BuildTickets(ctx context.Context, r shared.Runner, cfg *shared.Config) ([]Ticket, error) {
