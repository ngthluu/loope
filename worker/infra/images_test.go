...
func TestDownloadIssueImagesRewritesAndSaves(t *testing.T) {
	...
	f := &testkit.FakeRunner{Handler: func(c testkit.RCall) (string, string, error) {
		if testkit.HasArg(c.Args, "token") { // gh auth token
	...
	if !testkit.HasArg(curl.Args, url) {
		t.Errorf("curl args missing url: %v", curl.Args)
	}
	if got := testkit.ArgAfter(curl.Args, "-H"); !strings.Contains(got, "gho_faketoken") {
...
func TestDownloadIssueImagesSkipsFailedDownload(t *testing.T) {
	...
	f := &testkit.FakeRunner{Handler: func(c testkit.RCall) (string, string, error) {
		if testkit.HasArg(c.Args, "token") {
			return "gho_faketoken\n", "", nil
		}
		if testkit.HasArg(c.Args, bad) {
