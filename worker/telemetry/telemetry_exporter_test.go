...
func TestExporterPushOnceIncludesFreshUsage(t *testing.T) {
	...
	snap := wire.UsageSnapshot{FiveHourUsedPct: 42, CapturedAt: time.Now()}
...
func TestExporterPushOnceReturnsErrorOnNonNoContent(t *testing.T) {
	...
	cfg := &shared.Config{RepoSlug: "o/r", WorkDir: "/work", Telemetry: &shared.TelemetryConfig{ServerURL: srv.URL, Token: "wrong", PushIntervalSec: 15}}
...
func TestScanIssueLogsReadsFilesFromEachDir(t *testing.T) {
	...
	byName := map[string]wire.IssueLogDir{}
	for _, d := range dirs {
		byName[d.Name] = d
	}
...
func TestExporterPushOnceIncludesIssueLogs(t *testing.T) {
	...
	cfg := &shared.Config{RepoSlug: "o/r", WorkDir: workDir, Telemetry: &shared.TelemetryConfig{ServerURL: srv.URL, Token: "secret", PushIntervalSec: 15}}
	e := &TelemetryExporter{cfg: cfg, client: srv.Client(), tailer: &LogTailer{path: filepath.Join(t.TempDir(), "daemon.log")}}
...
func TestReadUsageSnapshotStaleReturnsNil(t *testing.T) {
	...
	stale := wire.UsageSnapshot{CapturedAt: time.Now().Add(-31 * time.Minute)}
...
func TestApplyIssueLogBudgetElidesOldestContentFirst(t *testing.T) {
	...
	dirs := []wire.IssueLogDir{
		{Name: "issue-old", Files: []wire.IssueLogFile{{Name: "f", Content: "0123456789", ModTime: old}}},
		{Name: "issue-new", Files: []wire.IssueLogFile{{Name: "f", Content: "0123456789", ModTime: fresh}}},
	}
